...
//log on notify msg q
        if(notify_msgq >= 0 ) {
                num_bytes = mq_timedsend(notify_msgq,
                                         (const char*)&err_pack,
                                         sizeof(err_pack),
                                         prio,
                                         &expire);
                if(num_bytes<0) {perror("mq_send to notify_msgq in notify");}
        }
        if(logger_msgq >= 0) {
...
/*This thread is invoked via mq_notify on main once any initialization or error message is put on notify_msgq
 */
void notifyRcvThread(union sigval sv){
//read and print the error
        if(sv.sival_ptr == NULL) {printf("notifyRcvThread argument\n"); return;}
...
//empty the logger_msgq - notification only for an empty q
        ...
        if(ret !=0 ) {perror("notifyRcvThread-mutex unlock"); return;}
